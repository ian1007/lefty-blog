...
$(function () {
  if (document.body.contains(document.querySelector('.service__contact'))) {
    (function () {
      const constraints = {
        'name': { // 來信的人
          presence: {
      ...
      const submitButton = document.querySelector('.service__contact button');
      // validate
      // 1. 防止表單在驗證之前傳送
      ...
      form.addEventListener('submit', function (e) {
        ...
        if (allpass) {
          $('.service__contact .btn__icon').remove();
          $('.service__contact button').prepend('<div class="sk-chase"><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div></div>');
          const name = $('#contact__name').val();
          const email = $('#contact__email').val();
          const description = $('#contact__description').val();
          $.ajax({
            url: '/contact',
            method: 'POST',
            headers: {
            ...
            form.remove();
            $('span#contacted').removeClass('d-none');
          }).fail(function() {
            form.remove();
            $('span#contactError').removeClass('d-none');
          });
        }
        else {
      ...
      // 2. 監聽 input 值改變的狀況
      const inputs = document.querySelectorAll('.service__contact input, .service__contact textarea');
