    ...
    $('.btn__more').each(function () {
      $(this).popover({
        ...
        content: function () { // 塞入的內容
          return $('#categories__more').html();
        },
      }).on('show.bs.popover', function () { // 當被點擊後，會被馬上啟動
        // 傳遞參數給 popover
        const categoryPath = $(this).attr('data-path');
        const categoryId = $(this).attr('data-id');
        $('#categories__more').find('a').attr('href', '/admin/category/' + categoryPath);
        $('#categories__more').find('button').attr('data-id', categoryId);
        // 傳遞參數給 modal
        $('#deleteConfirm').attr('data-id', categoryId);
